...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public PoetryResult<UserVO> regist(UserVO user) {
        ...
        Integer count = Math.toIntExact(lambdaQuery().eq(User::getUsername, user.getUsername()).count());
        if (count != 0) {
            return PoetryResult.fail("用户名重复！");
        }
        if (StringUtils.hasText(user.getPhoneNumber())) {
            Integer phoneNumberCount = Math.toIntExact(lambdaQuery().eq(User::getPhoneNumber, user.getPhoneNumber()).count());
            if (phoneNumberCount != 0) {
                return PoetryResult.fail("手机号重复！");
            }
        } else if (StringUtils.hasText(user.getEmail())) {
            Integer emailCount = Math.toIntExact(lambdaQuery().eq(User::getEmail, user.getEmail()).count());
            if (emailCount != 0) {
    ...
    public PoetryResult<UserVO> updateUserInfo(UserVO user) {
        if (StringUtils.hasText(user.getUsername())) {
            ...
            Integer count = Math.toIntExact(lambdaQuery().eq(User::getUsername, user.getUsername()).ne(User::getId, PoetryUtil.getUserId()).count());
            if (count != 0) {
                return PoetryResult.fail("用户名重复！");
            }
        }
    ...
    public PoetryResult getCode(Integer flag) {
        ...
        if (flag == 1) {
            ...
            if (count == null || count.get() < CommonConst.CODE_MAIL_COUNT) {
                mailUtil.sendMailMessage(mail, "您有一封来自" + (webInfo == null ? "智澈乐尚网络工作平台" : webInfo.getWebTitle()) + "的回执！", text);
                if (count == null) {
    ...
    public PoetryResult<UserVO> updateSecretInfo(String place, Integer flag, String code, String password) {
        ...
        if (flag == 1) {
            Integer count = Math.toIntExact(lambdaQuery().eq(User::getPhoneNumber, place).count());
            if (count != 0) {
                return PoetryResult.fail("手机号重复！");
            }
            ...
            Integer count = Math.toIntExact(lambdaQuery().eq(User::getEmail, place).count());
            if (count != 0) {
                return PoetryResult.fail("邮箱重复！");
            }
            Integer codeCache = (Integer) PoetryCache.get(CommonConst.USER_CODE + PoetryUtil.getUserId() + "_" + place + "_" + flag);
    ...
    private String getCodeMail(int i) {
        ...
        String webTitle = (webInfo == null ? "智澈乐尚网络工作平台" : webInfo.getWebTitle());
