<template>
  <div>
    <div v-if="!common.isEmpty(article)">
      ...
      <div class="article-head my-animation-slide-top">
        ...
        <div class="article-info-news" @click="weiYanDialogVisible = true"
          v-if="!common.isEmpty(store.currentUser) && store.currentUser.id === article.userId">
    ...
    <el-dialog title="版权声明" v-model="copyrightDialogVisible" width="80%" :append-to-body="true" class="article-copy"
      center>
      <div style="display: flex;align-items: center;flex-direction: column">
        <el-avatar shape="square" :size="35" :src="store.webInfo.avatar"></el-avatar>
        <div class="copyright-container">
          <p>
            {{ store.webInfo.webName }}是指运行在{{ $constant.host }}域名及相关子域名上的网站，本条款描述了{{ store.webInfo.webName }}的网站版权声明：
          </p>
          <ul>
            <li>
              {{ store.webInfo.webName }}提供的所有文章、展示的图片素材等内容部分来源于互联网平台，仅供学习参考。如有侵犯您的版权，请联系{{ store.webInfo.webName
              }}负责人，{{ store.webInfo.webName }}承诺将在一个工作日内改正。
            </li>
            <li>
              {{ store.webInfo.webName }}不保证网站内容的全部准确性、安全性和完整性，请您在阅读、下载及使用过程中自行确认，{{ store.webInfo.webName
              }}亦不承担上述资源对您造成的任何形式的损失或伤害。
            </li>
            <li>未经{{ store.webInfo.webName }}允许，不得盗链、盗用本站内容和资源。</li>
            <li>
              {{ store.webInfo.webName }}旨在为广大用户提供更多的信息；{{ store.webInfo.webName
              }}不保证向用户提供的外部链接的准确性和完整性，该外部链接指向的不由本站实际控制的任何网页上的内容，{{ store.webInfo.webName }}对其合法性亦概不负责，亦不承担任何法律责任。
            </li>
            <li>
              {{ store.webInfo.webName
              }}中的文章/视频（包括转载文章/视频）的版权仅归原作者所有，若作者有版权声明或文章从其它网站转载而附带有原所有站的版权声明者，其版权归属以附带声明为准；文章仅代表作者本人的观点，与{{
                store.webInfo.webName }}立场无关。
            </li>
            <li>
              {{ store.webInfo.webName }}自行编写排版的文章均采用
...
const process = defineAsyncComponent(() => import('./common/process'))
const videoPlayer = defineAsyncComponent(() => import('./common/videoPlayer'))
...
// 引入封装的 composables
import { getSubscribe_api } from "@/api/public"

import { deleteWeiYan_api, listNews_api, saveNews_api } from "@/api/weiYan"

import { getArticleById_api } from "@/api/article"
...
// 监听页面滚动（等价于 mounted 和 destroyed）
const onScrollPage = () => {
  scrollTop.value = document.documentElement.scrollTop || document.body.scrollTop
  if (scrollTop.value < window.innerHeight / 4) {
...
onMounted(() => {
  ...
  if (!isEmpty(id.value)) {
    ...
    if (localStorage.getItem("showSubscribe") !== "0") {
      ElNotification({
        title: '文章订阅',
        type: 'success',
...
// 监听 scrollTop 变化调整 toc 按钮位置
watch(scrollTop, (newVal) => {
  let isShow = newVal - window.innerHeight > 30
  $("#toc-button").css("bottom", isShow ? "14.1vh" : "8vh")
})
...
const clickTocButton = () => {
  const display = $(".toc")
  display.css("display", display.css("display") === "none" ? "unset" : "none")
}
...
const subscribeLabel = async () => {
  ...
  const confirmText = subscribe.value ? '取消订阅' : '订阅'
  const message = `确认${confirmText}专栏【${article.label.labelName}】？` +
    (subscribe.value ? "" : "订阅专栏后，该专栏发布新文章将通过邮件通知订阅用户。")

  try {
    await ElMessageBox.confirm(message, confirmText, {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      center: true
    })

    const res = await getSubscribe_api({
 ...
 * 删除微言
 */
const deleteTreeHole = async (treeHoleId) => {
  if (isEmpty(store.currentUser)) {
    ElMessage.error("请先登录！")
    return
  }
  ...
  try {
    ...
    await deleteWeiYan_api({ id: treeHoleId })
 ...
 * 获取文章详情
 */
const getArticle = async (pwd) => {
  try {
    const res = await getArticleById_api({ id: id.value, password: pwd })
    if (!common.isEmpty(res.data)) {
      Object.assign(article, res.data)
      getNews()
      const md = new MarkdownIt({ breaks: true }).use(require('markdown-it-multimd-table'))
      articleContentHtml.value = md.render(article.articleContent)
      await nextTick()
      imgShow(".entry-content img")
      highlight()
      addId()
      getTocbot()
      if (!common.isEmpty(pwd)) {
        localStorage.setItem("article_password_" + id.value, pwd)
      }
      showPasswordDialog.value = false

      if (!common.isEmpty(store.currentUser) && !common.isEmpty(store.currentUser.subscribe)) {
        subscribe.value = JSON.parse(store.currentUser.subscribe).includes(article.labelId);
      }

    }
  } catch (error) {
    if (error.message.startsWith("密码错误")) {
      localStorage.removeItem(`article_password_${id.value}`)
...
const getTocbot = () => {
  ...
  script.src = constant.tocbot // Vue3 中用 import 的常量
...
const highlight = () => {
  $("pre").each((i, item) => {
    ...
    // 检测语言
    let lang = (preCode.attr("class") || "")
      .split(" ")
      .find(cls => cls.startsWith("language-"))
      ?.replace("language-", "")

    if (!hljs.getLanguage(lang)) {
      const autoLang = hljs.highlightAuto(preCode.text())
      lang = autoLang.language || "plaintext"
    ...
    // 设置 data-rel 和 lang class
    preCode.attr("data-rel", lang.toUpperCase())
  ...
  $("pre code").each((i, block) => {
    ...
    $(block).after(`<a class="copy-code" href="javascript:" data-clipboard-target="#hljs-${i}"><i class="fa fa-clipboard"></i></a>`)
