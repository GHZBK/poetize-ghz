<template>
  <div>
    <Loader :loading="loading">
      ...
      <template #body>
        <!-- 首页图片 -->
        <el-image style="animation: header-effect 2s" class="background-image-index" v-once lazy :src="!common.isEmpty(store.webInfo.backgroundImage)
          ? store.webInfo.backgroundImage
          : store.webInfo.randomCover[Math.floor(Math.random() * store.webInfo.randomCover.length)]" fit="cover">
          <template #error>
        ...
        <div class="signature-wall myCenter my-animation-hideToShow">
          <h1 class="playful">
            <span v-for="(a, index) in store.webInfo.webTitle" :key="index">{{ a }}</span>
          </h1>
          ...
          <el-icon class="el-icon-arrow-down">
            <ArrowDownBold @click="navigation('.page-container-wrap')" />
          </el-icon>
        </div>
        <!-- 首页内容 -->
          ...
          <div class="page-container">
            ...
            <div class="recent-posts">
              <div class="announcement background-opacity">
                ...
                <div>
                  <div v-for="(notice, index) in common.pushNotification(store.webInfo.notices, true)" :key="index">
                    {{ notice }}
                  </div>
                </div>
              </div>
    ...
    <el-dialog v-model="pushDialogVisible" title="每日推荐" width="60%" append-to-body :close-on-click-modal="false"
      class="index-push" center>
...
// ✅ Vue3 响应式
import { reactive, ref, computed, onMounted, onBeforeMount, nextTick, defineAsyncComponent } from "vue";

import { getArticles_api, getSortArticles_api } from "@/api/article"
...
const articles = ref([]);
const sortArticles = reactive({});

console.log(store.getters, 45)
...
onMounted(() => {
  setTimeout(() => {
    const notices = store.webInfo?.notices || [];
    Object.assign(push, common.pushNotification(notices, false));

    if (!common.isEmpty(push)) {
      if ("0" !== localStorage.getItem("showPushNotification_" + push["链接"])) {
        pushDialogVisible.value = true;
        localStorage.setItem("showPushNotification_" + push["链接"], "0");
      }
    }
  }, 2000);
});
...
async function selectArticle(articleSearch) {
  pagination.current = 1;
  pagination.size = 10;
  pagination.total = 0;
  pagination.searchKey = "";
  pagination.sortId = null;
  pagination.articleSearch = articleSearch;
  articles.value = [];
  await getArticles();
  nextTick(() => {
    indexType.value = 2;
...
async function getSortArticles() {
  try {
    const res = await getSortArticles_api();
    if (!common.isEmpty(res.data)) {
      Object.assign(sortArticles, res.data);
    }
  }
  catch (error) {
    ElMessage.error(error.message);
...
function getGuShi() {
  printerInfo.value = `知识如海波澜壮，探索之心永不息，
实验之火燃心中，真理必定在前方。`;
}
</script>
